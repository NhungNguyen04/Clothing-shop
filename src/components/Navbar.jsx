...
import { BiBell, BiCart, BiMessage, BiNotification } from 'react-icons/bi';
    ...
    <Link to="/">
      ...
      <div className='hidden sm:flex items-center gap-6'>
        <Link to="/cart" className='cursor-pointer relative'>
          <BiCart className='text-2xl text-pink-500' />
          {cartItemCount > 0 && (
        ...
        <Link to="/chats" className='cursor-pointer relative'>
          <MessageCircle className='text-2xl text-pink-500' />
        </Link>

        <NotificationDropdown>
          <div className='cursor-pointer relative'>
            <BiBell className='text-2xl text-pink-500' />
            {unreadCount > 0 && (
              <div className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-red-500 flex items-center justify-center">
                <p className="text-white text-xs">{unreadCount > 99 ? '99+' : unreadCount}</p>
              </div>
            )}
          </div>
        </NotificationDropdown>
      ...
      {visible && (
        <div className='sm:hidden absolute left-0 top-full w-full bg-white shadow-md z-40 p-4'>
          <ul className='flex flex-col gap-2'>
            {['HOME', 'COLLECTION', 'ORDERS', 'CHATS', 'NOTIFICATION', 'PROFILE'].map((item) => (
